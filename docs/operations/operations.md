...
| 层级 | 说明 | 示例 |
|------|------|------|
| `WORKSPACE_ROOT` | Hermes Agent 工作区根目录 | `D:\wk\novels` |
| `.hermes/` | 工作区级配置与项目指针 | `D:\wk\novels\.hermes\` |
| `PROJECT_ROOT` | 某本书的项目根目录（由 `/webnovel-init` 创建） | `D:\wk\novels\凡人资本论` |
| `SKILL_ROOT_DIR` | 插件缓存目录（不在项目内，由 Marketplace 安装管理） | 自动管理 |
...
插件安装在 Hermes 插件缓存目录，不在书项目内。运行时通过 `SKILL_ROOT_DIR` 引用：
...
```text
${HERMES_HOME:-~/.hermes}/webfission-creative/workspaces.json
```
...
```bash
python -X utf8 "${SKILL_ROOT_DIR}/scripts/webnovel.py" --project-root "${WORKSPACE_ROOT}" preflight
```
...
```bash
pwsh "${SKILL_ROOT_DIR}/scripts/run_tests.ps1" -Mode smoke
pwsh "${SKILL_ROOT_DIR}/scripts/run_tests.ps1" -Mode full
```
