...
- 它为什么能稳定工作，而不是只靠文案堆砌
- 其中哪些思想和架构值得 `webfission-creative` 学习
- 哪些做法可以借鉴，哪些不能直接照搬

说明：

- 本次调研基于本地源码目录  
  `C:\Users\lcy\.gemini\tmp\webfission-creative\ui-ux-pro-max-skill\src\ui-ux-pro-max`
...
`webfission-creative/scripts/reference_search.py` 目前仍然是**全局硬编码字段**，而不是 per-domain 注册：
...
- `templates/platforms/hermes.json`
...
| `evolution-spec` 六层 | `ui-ux-pro-max` 参照物 | `webfission-creative` 现状 / 目标 | 当前完成度 |
|------|------|------|------|
...
对 `webfission-creative` 来说，最值得学习的最终不是：
