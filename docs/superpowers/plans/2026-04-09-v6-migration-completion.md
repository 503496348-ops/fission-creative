...
**Tech Stack:** Python 3.13, pytest, SQLite (state.json + index.db), Hermes plugin (markdown skills/agents)
...
Run: `cd "D:/wk/novel skill/webfission-creative" && python -m pytest webfission-creative/scripts -x --tb=short --no-cov`
Expected: 全部通过
...
git add webfission-creative/scripts/data_modules/index_manager.py webfission-creative/scripts/data_modules/tests/test_data_modules.py webfission-creative/scripts/data_modules/tests/test_webnovel_unified_cli.py
...
Run: `cd "D:/wk/novel skill/webfission-creative" && grep -r "golden_three" webfission-creative/ --include="*.py" --include="*.md" | grep -v golden_three_checker.py`
...
```bash
cd "D:/wk/novel skill/webfission-creative"
git rm webfission-creative/scripts/golden_three_checker.py
```
...
```bash
cd "D:/wk/novel skill/webfission-creative"
git add webfission-creative/scripts/data_modules/webnovel.py webfission-creative/scripts/data_modules/tests/test_prompt_integrity.py webfission-creative/skills/webnovel-write/references/polish-guide.md webfission-creative/references/reading-power-taxonomy.md
git commit -m "$(cat <<'EOF'
...
Run: `cd "D:/wk/novel skill/webfission-creative" && python -m pytest webfission-creative/scripts/data_modules/tests/test_context_manager.py -x --tb=short --no-cov`
Expected: 通过
...
Run: `cd "D:/wk/novel skill/webfission-creative/webfission-creative/scripts" && python -m pytest data_modules/tests/test_chapter_status.py -v --no-cov`
...
```bash
cd "D:/wk/novel skill/webfission-creative"
git add webfission-creative/scripts/data_modules/state_manager.py webfission-creative/scripts/data_modules/tests/test_chapter_status.py
git commit -m "$(cat <<'EOF'
...
Run: `cd "D:/wk/novel skill/webfission-creative" && python -m pytest webfission-creative/scripts --tb=short`
Expected: 全部通过
...
grep -rn "continuity-checker\|setting-checker\|ooc-checker\|high-point-checker\|pacing-checker\|reader-pull-checker" \
  webfission-creative/skills/ webfission-creative/agents/ webfission-creative/scripts/*.py \
  --include="*.md" --include="*.py" || echo "PASS: 无旧 checker 引用"

# 标准 2: 审查路径唯一
echo "--- 标准 2: 审查路径 ---"
grep -l "reviewer" webfission-creative/skills/webnovel-write/SKILL.md webfission-creative/skills/webnovel-review/SKILL.md && echo "PASS: 均走 reviewer"

# 标准 3: workflow_manager 移除
echo "--- 标准 3: workflow_manager ---"
test ! -f webfission-creative/scripts/workflow_manager.py && echo "PASS: 文件已删除" || echo "FAIL: 文件仍存在"

# 标准 4: legacy 术语（运行时路径）
echo "--- 标准 4: legacy 术语 ---"
grep -rn "timeline_gate\|_aggregate_checker\|_normalize_checker\|_build_timeline_gate" \
  webfission-creative/scripts/ --include="*.py" | grep -v "test_" | grep -v "__pycache__" || echo "PASS: 运行时无 legacy"

# 标准 6: 章节状态模型
echo "--- 标准 6: 章节状态 CLI ---"
cd webfission-creative/scripts && python -X utf8 data_modules/state_manager.py --help 2>&1 | grep -q "get-chapter-status" && echo "PASS: CLI 已注册" || echo "FAIL: CLI 未注册"
```
