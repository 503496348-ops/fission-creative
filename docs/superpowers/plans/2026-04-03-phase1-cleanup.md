...
**Tech Stack:** Python 3.13, pytest, Hermes Agent plugin (markdown agents/skills)
...
| 文件 | 原因 |
|------|------|
| `scripts/workflow_manager.py` | Hermes Agent 原生 /resume 替代 |
...
Run: `cd webfission-creative/scripts && python -m pytest data_modules/tests/test_review_schema.py -v --no-cov`
...
Run: `cd "D:\wk\novel skill\webfission-creative" && python -m pytest --no-cov --tb=short`
...
```bash
git add -A
git commit -m "refactor: 移除 workflow_manager + resume skill，由 Hermes Agent /resume 替代"
```
...
Run: `cd webfission-creative/scripts && python -m pytest data_modules/tests/test_webnovel_unified_cli.py::test_review_pipeline_builds_artifacts_v6 -v --no-cov`
...
Run: `cd webfission-creative/scripts && python -m pytest data_modules/tests/test_webnovel_unified_cli.py -v --no-cov`
Expected: 全部通过
...
Run: `cd "D:\wk\novel skill\webfission-creative" && python -m pytest --tb=short`
...
```bash
# 这些文件应该不存在
test ! -f webfission-creative/scripts/workflow_manager.py
test ! -f webfission-creative/skills/webnovel-resume/SKILL.md
test ! -f webfission-creative/agents/consistency-checker.md
test ! -f webfission-creative/agents/continuity-checker.md
test ! -f webfission-creative/agents/ooc-checker.md
test ! -f webfission-creative/agents/high-point-checker.md
test ! -f webfission-creative/agents/pacing-checker.md
test ! -f webfission-creative/agents/reader-pull-checker.md
test ! -f webfission-creative/references/checker-output-schema.md

# 这些文件应该存在
test -f webfission-creative/agents/reviewer.md
test -f webfission-creative/references/review-schema.md
test -f webfission-creative/scripts/data_modules/review_schema.py
```

- [ ] **Step 2: 全量测试 + 覆盖率**

Run: `cd "D:\wk\novel skill\webfission-creative" && python -m pytest`
Expected: 全部通过，覆盖率 ≥ 90%

- [ ] **Step 3: 确认 agents/ 目录只剩 3 个 agent**

```bash
ls webfission-creative/agents/
