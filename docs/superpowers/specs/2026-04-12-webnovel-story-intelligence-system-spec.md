...
当前 `webfission-creative` 已经具备以下能力：
...
对 `webfission-creative` 来说，真正需要学习的是这条系统链，而不是“多几个 CSV”。
...
- `SKILL_ROOT` 是插件安装目录，负责承载代码、skills、scripts、references
- `PROJECT_ROOT` 是真实书项目根目录，定义为包含 `.webnovel/state.json` 的目录
- `.story-system/` 必须落在 `PROJECT_ROOT` 下，而不是 `SKILL_ROOT`
...
这就是 `webfission-creative` 从“离散检索工具”升级为“系统级智能写作底座”的关键跃迁。
