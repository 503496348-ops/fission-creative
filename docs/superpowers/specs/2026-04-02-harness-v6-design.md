...
- **Hermes Agent 本身就是 harness**——不另建编排层，充分利用原生能力（/resume、Task、子 agent 隔离、自动 compaction）
...
| 16 | workflow_manager + resume skill 浪费 | Hermes Agent 原生 /resume 即可恢复中断会话 |
...
| 废弃 | 替代 |
|------|------|
| workflow_manager.py | Hermes Agent 原生 /resume |
| resume skill | Hermes Agent 原生 /resume |
