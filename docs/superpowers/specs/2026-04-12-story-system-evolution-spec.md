...
- 当前这个仓库只是**插件源码开发目录**
- 它不是插件安装后的运行时根目录
- story system 的落盘路径必须基于**Hermes Agent 安装后的真实运行目录模型**来定义
...
1. `SKILL_ROOT`
   ...
   - **不允许**在这里写入小说项目数据
2. `WORKSPACE_ROOT`
   - 当前工作区根目录，可通过 `.hermes/.webnovel-current-project` 指针解析当前书项目
3. `PROJECT_ROOT`
   - **书项目根目录**
...
- 指针文件 = `D:\wk\xiaoshuo\.hermes\.webnovel-current-project`
