#!/usr/bin/env python3
"""
Project location helpers for webfission-creative scripts.
...
CURRENT_PROJECT_POINTER_REL: Path = Path(".hermes") / ".webnovel-current-project"

# 用户级全局映射（当 skills/agents 安装在 ~/.hermes 时，项目目录可能在任意盘符）
# 该文件用于在“空上下文 + CWD 不在项目内”的情况下仍能定位到正确 project_root。
GLOBAL_REGISTRY_REL: Path = Path("webfission-creative") / "workspaces.json"

# Hermes Agent 常见环境变量（存在时优先作为“工作区根目录”提示）
ENV_WORKSPACE_ROOT = "WORKSPACE_ROOT"
ENV_HERMES_HOME = "HERMES_HOME"
ENV_WEBNOVEL_HERMES_HOME = "WEBNOVEL_HERMES_HOME"
...
def _get_user_hermes_root() -> Path:
    raw = os.environ.get(ENV_WEBNOVEL_HERMES_HOME) or os.environ.get(ENV_HERMES_HOME)
    if raw:
        try:
...
def _global_registry_path() -> Path:
    return _get_user_hermes_root() / GLOBAL_REGISTRY_REL
...
def _resolve_project_root_from_global_registry(
    ...
    *,
    ...
    安全策略：
    - 优先使用 workspace_hint / WORKSPACE_ROOT 提示做匹配。
    ...
    hints: list[Path] = []
    env_ws = os.environ.get(ENV_WORKSPACE_ROOT)
    if env_ws:
...
def _resolve_project_root_from_pointer(cwd: Path, *, stop_at: Optional[Path] = None) -> Optional[Path]:
    ...
    - relative path is also supported (resolved relative to pointer's `.hermes/` dir).
    """
...
def _find_workspace_root_with_hermes(start: Path) -> Optional[Path]:
    """Find nearest ancestor containing `.hermes/`."""
...
def write_current_project_pointer(project_root: Path, *, workspace_root: Optional[Path] = None) -> Optional[Path]:
    ...
    If no workspace root with `.hermes/` can be found, returns None (non-fatal).
    """
    root = normalize_windows_path(project_root).expanduser().resolve()
    if not _is_project_root(root):
        raise FileNotFoundError(f"Not a webnovel project root (missing .webnovel/state.json): {root}")

    ws_root = Path(workspace_root).expanduser().resolve() if workspace_root else _find_workspace_root_with_hermes(root)
    if ws_root is None:
        ws_root = _find_workspace_root_with_hermes(Path.cwd().resolve())
    if ws_root is None:
        # 兜底：若无法找到 `.hermes/`，将项目父目录视为“工作区”候选，
        # 仅用于写入用户级 registry（不创建 `.hermes/` 目录，不写 pointer 文件）。
        ws_root = root.parent if root.parent != root else None
    # 注意：ws_root 可能为 None（例如全局安装的 skills/agents，工作区内没有 `.hermes/`）。
    # 这类情况仍然需要写入用户级 registry，以支持后续“空上下文”定位。

    pointer_file: Optional[Path] = None
    if ws_root is not None:
        # 仅当工作区内已经存在 `.hermes/` 时才写入指针，避免在任意目录下“凭空创建 .hermes/”。
        if (ws_root / ".hermes").is_dir():
            try:
...
def resolve_project_root(explicit_project_root: Optional[str] = None, *, cwd: Optional[Path] = None) -> Path:
    ...
    if explicit_project_root:
        ...
        # 兼容：显式传入“工作区根目录”（含 `.hermes/.webnovel-current-project` 指针）
        ...
        # 兼容：显式传入“工作区根目录”但其 `.hermes/` 在用户目录（全局安装）时，
    ...
    # Workspace pointer fallback (for layouts where `.hermes` is in workspace root and projects are subdirs).
    pointer_root = _resolve_project_root_from_pointer(base, stop_at=git_root)
    if pointer_root is not None:
        return pointer_root

    # 用户级 registry fallback（仅在“有上下文提示”时启用，避免误命中）
    # - 若 WORKSPACE_ROOT 存在：认为 Hermes Agent 提供了工作区上下文
    # - 否则仅在 base 位于某个已记录 workspace 内时启用（前缀匹配）
    allow_last_used = bool(os.environ.get(ENV_WORKSPACE_ROOT))
    reg_root = _resolve_project_root_from_global_registry(
        base,
        workspace_hint=None,
