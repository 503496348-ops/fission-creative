"""Optional FastAPI service for fission-creative diagnostics."""
...
from fission_creative_history_store import latest_runs, save_run
...
def create_app():
    ...
    app = FastAPI(title='fission-creative', version='0.1.0')

    @app.get('/health')
    def health():
        return {'status': 'ok', 'service': 'fission-creative'}
...
def main() -> int:
    if FastAPI is None:
        raise RuntimeError('请先安装 fastapi 与 uvicorn（pip install fastapi uvicorn）')
    parser = argparse.ArgumentParser(description='fission-creative API')
