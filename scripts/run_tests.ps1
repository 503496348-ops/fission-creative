...
try:
    d = Path(tempfile.mkdtemp(prefix="webfission_creative_pytest_"))
...
if ($Mode -eq "smoke") {
    python -m pytest -q `
        .hermes/scripts/data_modules/tests/test_extract_chapter_context.py `
        .hermes/scripts/data_modules/tests/test_rag_adapter.py `
        --basetemp $baseTemp `
        --no-cov `
        -p no:cacheprovider
    exit $LASTEXITCODE
...
python -m pytest -q `
    .hermes/scripts/data_modules/tests `
    --basetemp $baseTemp `
    -p no:cacheprovider
exit $LASTEXITCODE
