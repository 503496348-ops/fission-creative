...
version: 6.0.0
tags: [writing, novel, dashboard, visualization]
...
if [ -z "${SKILL_ROOT_DIR}" ] || [ ! -d "${SKILL_ROOT_DIR}/dashboard" ]; then
  echo "ERROR: 未找到 dashboard 模块: ${SKILL_ROOT_DIR}/dashboard" >&2
  exit 1
fi

export DASHBOARD_DIR="${SKILL_ROOT_DIR}/dashboard"
```
...
if [ -n "${PYTHONPATH:-}" ]; then
  export PYTHONPATH="${SKILL_ROOT_DIR}:${PYTHONPATH}"
else
  export PYTHONPATH="${SKILL_ROOT_DIR}"
fi
