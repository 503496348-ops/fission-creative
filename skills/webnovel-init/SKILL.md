...
version: 6.0.0
tags: [writing, novel, initialization, story-system]
requires_tools: [read_file, write_file, search_files, terminal, clarify]
...
- `Agent`：拆分并行子任务（如题材映射、约束包候选生成、文件验证）；Step 1.5 用户选择参考书拆解作为灵感来源时，调用 `webfission-creative:deconstruction-agent`。
...
if [ -z "${SKILL_ROOT_DIR}" ] || [ ! -d "${SKILL_ROOT_DIR}/scripts" ]; then
  echo "ERROR: 未设置 SKILL_ROOT_DIR 或缺少目录: ${SKILL_ROOT_DIR}/scripts" >&2
  exit 1
fi
export SCRIPTS_DIR="${SKILL_ROOT_DIR}/scripts"
```

必须做：
- 确认当前目录可写。
- 解析脚本目录并确认入口存在（仅支持插件目录）：
  - 固定路径：`${SKILL_ROOT_DIR}/scripts`
...
当用户选择参考作品拆书且提供文本路径或章节摘录时，必须使用 `Agent` 工具调用 `webfission-creative:deconstruction-agent`，不得由 init 主流程口头替代拆解结果。

```text
Agent(
  subagent_type: "webfission-creative:deconstruction-agent",
...
| 陷阱 | 说明 | 规避方法 |
|------|------|---------|
| 总纲字段缺失 | 交互跳步导致总纲关键字段为空 | 每阶段结束前校验必填字段，缺失则重新询问 |
| 设定集与总纲矛盾 | 角色设定与总纲主线冲突 | init_project.py 自动交叉校验，冲突时暂停 |
| idea_bank.json 不一致 | 选定方案与实际写入不匹配 | 写入后立即读取验证，hash比对 |
| .story-system 未初始化 | 后续 write/plan 报错找不到合同目录 | init 结束前必须验证 .story-system/MASTER_SETTING.json 存在 |
| 编码问题 | 中文路径/文件名在某些终端乱码 | 全程使用 UTF-8，脚本加 `python -X utf8` |

## Verification（验证标准）

```bash
# 验证项目结构完整
test -f .webnovel/state.json && echo "✅ state.json"
test -d .story-system && echo "✅ .story-system"
test -f 设定集/主角卡.md && echo "✅ 主角卡"
test -f 大纲/总纲.md && echo "✅ 总纲"

# 验证 state.json 可解析
python3 -c "import json; json.load(open('.webnovel/state.json')); print('✅ state.json valid')"
```
