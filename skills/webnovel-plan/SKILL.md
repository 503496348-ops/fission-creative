...
version: 6.0.0
tags: [writing, novel, outline, planning]
...
export SKILL_ROOT="${SKILL_ROOT_DIR}/skills/webnovel-plan"
...
| 陷阱 | 说明 | 规避方法 |
|------|------|---------|
| 卷纲与总纲冲突 | 卷末高潮与总纲主线矛盾 | 生成后自动与总纲交叉比对 |
| 时间线回跳 | 章节时间戳非递增（非闪回） | 检测到回跳必须标注闪回或修正 |
| 章纲CBN/CEN断裂 | 相邻章节CEN→下一章CBN不承接 | 链式校验每对相邻章节 |
| 设定增量未写回 | 新出现的势力/能力未入库 | plan结束后扫描新增实体并提示 |
| 节拍表为空 | 跳过节拍直接写章纲 | 硬失败，必须先完成节拍表 |

## Verification（验证标准）

```bash
# 验证卷纲存在且非空
test -s 大纲/卷纲/卷{N}.md && echo "✅ 卷纲"

# 验证章纲数量匹配
ls 大纲/章纲/卷{N}/ | wc -l  # 应等于卷纲中的章节数

# 验证时间线
python -X utf8 "${SCRIPTS_DIR}/webnovel.py" --project-root "${PROJECT_ROOT}" \
  story-system timeline validate --volume {N}
```
