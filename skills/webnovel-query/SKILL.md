...
version: 6.0.0
tags: [writing, novel, query, lookup]
...
export SKILL_ROOT="${SKILL_ROOT_DIR}/skills/webnovel-query"
export PROJECT_ROOT="$(python "${SCRIPTS_DIR}/webnovel.py" --project-root "${WORKSPACE_ROOT}" where)"
```

- `PROJECT_ROOT` 必须包含 `.webnovel/state.json`
- **禁止**在 `${SKILL_ROOT_DIR}/` 下读取或写入项目文件
...
- 若数据源缺失，明确告知用户缺少什么文件
- 若查询无匹配，返回空结果并建议检查范围
- 若 `.story-system/` 合同与 accepted commit 缺失，必须显式说明当前查询已降级到 legacy fallbackSKILLEOF
