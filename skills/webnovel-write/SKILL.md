...
version: 6.0.0
tags: [writing, novel, chapter, drafting]
...
export SKILL_ROOT="${SKILL_ROOT_DIR:?}/skills/webnovel-write"
...
```text
Agent(
  subagent_type: "webfission-creative:context-agent",
...
```text
Agent(
  subagent_type: "webfission-creative:reviewer",
...
```text
Agent(
  subagent_type: "webfission-creative:data-agent",
...
| 陷阱 | 说明 | 规避方法 |
|------|------|---------|
| 上下文不足 | context-agent 输出的写作任务书信息不够 | 检查任务书字数和覆盖维度，不足则补充 |
| 审查未落库就润色 | 跳过reviewer直接润色 | 硬依赖：Step 3 必须在 Step 4 之前完成 |
| blocking=true 被忽略 | 审查发现阻断问题但未修复 | 充分性闸门强制检查 |
| Anti-AI 检测失败 | 润色后仍有明显AI味 | anti_ai_force_check=pass 是硬门槛 |
| Git 嵌套仓库 | 裸全量 git add 把父仓库嵌入 | backup 必须以 PROJECT_ROOT 为锚点 |
| 正文编码异常 | 中文标点/特殊字符被截断 | 全程 UTF-8，write_file 前检查编码 |

## Verification（验证标准）

```bash
# 验证正文文件存在且字数达标
wc -m 正文/第{NNNN}章-{title}.md  # 应 >= 2000 字

# 验证审查已落库
python -X utf8 "${SCRIPTS_DIR}/webnovel.py" --project-root "${PROJECT_ROOT}" \
  status --chapter {chapter_num} 2>&1 | grep -q "committed" && echo "✅ committed"

# 验证 CHAPTER_COMMIT 存在
ls .story-system/commits/CHAPTER_COMMIT-{chapter_num}-*.json && echo "✅ commit"
```
...
审查缺失→重跑 Step 3。摘要/状态/记忆缺失→重跑 Step 5。润色失真→回 Step 4 修复后重跑 Step 5。SKILLEOF
