...
version: 6.0.0
tags: [writing, novel, review, quality]
...
export SKILL_ROOT="${SKILL_ROOT_DIR}/skills/webnovel-review"
...
| 陷阱 | 说明 | 规避方法 |
|------|------|---------|
| 审查维度遗漏 | 六维审查只跑了部分 | 逐维度输出结果，缺失维度必须标注 |
| 审查报告未写入 index.db | 指标丢失，后续查询不到 | 报告生成后必须执行 review_metrics 写入 |
| blocking=true 未交互 | 阻断问题直接跳过未通知用户 | 硬规则：blocking=true 必须 AskUserQuestion |
| 审查过严/过松 | 标准不一致导致质量波动 | 使用统一 reviewer agent，不手动审查 |
| 章节不存在 | 审查不存在的章节编号 | 审查前先验证文件存在性 |

## Verification（验证标准）

```bash
# 验证审查报告存在
ls 审查报告/第{chapter_num}章-*.md && echo "✅ 报告"

# 验证 metrics 已写入
python -X utf8 "${SCRIPTS_DIR}/webnovel.py" --project-root "${PROJECT_ROOT}" \
  status --chapter {chapter_num} 2>&1 | grep -q "review" && echo "✅ metrics"
```
...
6. 如存在阻断问题，用户已明确选择处理策略。SKILLEOF
